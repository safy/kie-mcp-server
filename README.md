...
### Быстрые команды Claude Code

В `.claude/commands/` есть готовые команды:

```text
/nano-banana-pro оранжевая скульптура на чёрном фоне, 16:9
/seedance-2-5 скульптура медленно вращается, кинематографичный свет
```

Команды подставляют модель и базовые настройки автоматически. Чтобы они были
доступны во всех проектах, скопируй `.md` файлы в `~/.claude/commands/` и
перезапусти Claude Code. Это настройки клиента Claude Code: удалённый MCP-сервер
сам по себе не может добавлять slash-команды.
